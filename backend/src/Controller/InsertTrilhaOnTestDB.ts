 ...
 */
export default async function InsertTrilhaProduction(
  req: Request,
  res: Response
): Promise<void> {
  try {
 ...
 // Finalmente, ele envia uma resposta de sucesso com o número de trilhas adicionadas e os dados das trilhas inseridas.
    ...
    for (const trilha of trilhasToInsert) {
        ...
        const countResult = await connection('trilhas').count('id as count').first();
        ...
        await connection("trilhas").insert(formattedTrilha);
