 ...
 */
export default async function GetAllAssistancesFromUserAdressProduction(
  req: Request,
  res: Response
): Promise<void> {
  try {
    ...
    const user: User | undefined = await connection("users")
      .where({ id: userId })
    ...
    const assistances: Assistance[] = await connection("assistance")
