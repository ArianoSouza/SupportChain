 ...
 */
export default async function PostNewUserAnswersProduction(
  req: Request,
  res: Response
): Promise<void> {
  try {
    ...
    const existingAnswer: UserAnswer | undefined = await connection("useranswers")
      .where({
        fk_user_id: userId,
    ...
    await connection("useranswers").insert(newAnswer);
