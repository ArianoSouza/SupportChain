 ...
 */
export default async function GetQuestionsFromActivitieIdProduction(
  req: Request,
  res: Response
): Promise<void> {
  try {
    ...
    const questionary: Questionary | undefined = await connection("questionaries")
    ...
    const userAnswer: UserAnswer | undefined = await connection("useranswers")
      .where({
        fk_user_id: userId,
