 ...
 */
export default async function InsertAssistancesOnTestDB(
    req: Request,
    res: Response
): Promise<void> {
    try {
    ...
    // As assistências inseridas são armazenadas em 'insertedAssistances' para serem retornadas na resposta.
        ...
        for (const assistance of assistancesToInsert) {
            ...
            const countResult = await testDbConnection('assistance').count('id as count').first();
            ...
            await testDbConnection("assistance").insert(formattedAssistance);
