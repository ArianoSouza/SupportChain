...
import cloudinary from "../services/cloudnary";
 ...
 export default async function PostNewVideoOnTestDB(
  req: Request,
  res: Response
 ): Promise<void> {
  try {
  ...
  const countResult = await testDbConnection('videos').count('id as count').first();
  ...
  await testDbConnection("videos").insert(newVideo);
