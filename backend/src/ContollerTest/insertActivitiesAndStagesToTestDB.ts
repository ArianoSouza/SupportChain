...
export default async function InsertActivitiesaAndStagesNoTestDB(
    req: Request,
    res: Response
): Promise<void> {
    try {
        ...
        const activitiesCountResult = await testDbConnection('activities').count('id as count').first();
        ...
        await testDbConnection("activities").insert(formattedActivities);
        ...
        for (const stage of activitieStagesToInsert) {
            ...
            const stagesCountResult = await testDbConnection("activitie_stage").count('id as count').first();
        ...
        await testDbConnection("activitie_stage").insert(formattedStages);
