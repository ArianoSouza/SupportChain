...
export default async function insertNewUserRegisterToTestDB(
  req: Request,
  res: Response
): Promise<void> {
  try {
    ...
    const [existingUser] = await testDbConnection("users").where({ email });
    ...
    await testDbConnection("users").insert(newUser);
    ...
    await testDbConnection("clicktags").insert({
      id: clickTagId,
      fk_user_id: userId,
