 ...
 */
export default async function PostNewQuestionaryOnTestDB(
  req: Request,
  res: Response
): Promise<void> {
  try {
    ...
    const questCountResult = await testDbConnection('questionaries').count('id as count').first();
    ...
    await testDbConnection("questionaries").insert(newQuestionary);
