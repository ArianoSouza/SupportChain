 ...
 */
export default async function InsertTopicosNoTestDB(
  req: Request,
  res: Response
): Promise<void> {
  try {
 ...
 for (const topic of topicos) {
     ...
     const countResult = await testDbConnection('topics').count('id as count').first();
     ...
     await testDbConnection("topics").insert(formattedTopic);
